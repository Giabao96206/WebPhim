...
async function getEpisodeSourceFromAnimix(name, epNum) {
    var deprecated = true;
    if(deprecated) return "This function is deprecated, use getGogoanimeEpisodeSource insead"
    if(!name) throw new Error(`You must provide an anime name!`);
    if(!epNum) throw new Error(`episode number is required!`)
        let sources = [];
        let sources_bk = [];
        let type;
        let episodeGogoLink;
